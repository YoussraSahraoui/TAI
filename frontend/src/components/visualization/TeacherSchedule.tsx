...
interface Slot {
  roomName: string;
  subject: string;
  groups: string[];        // every curriculum attending this lecture
  type: string;
}

export default function TeacherSchedule({ grid }: Props) {
  ...
  // Auto-select the first teacher whenever the list (re)populates.
  useEffect(() => {
    if (teachers.length === 0) return;
    if (!selected || !teachers.includes(selected)) setSelected(teachers[0]);
  }, [teachers, selected]);
    ...
    for (const day of days) {
      ...
      for (const s of slots) {
        ...
        for (let ri = 0; ri < grid.rooms.length; ri++) {
          ...
          if (cell && cell.teacher === selected) {
            const allGroups = cell.curricula?.length
              ? cell.curricula
              : (cell.student_group ? [cell.student_group] : []);
            found = {
  ...
  if (teachers.length === 0) {
    return (
      <div className={styles.card}>
        <div className={styles.cardHeader}><h3>Schedule by Teacher</h3></div>
  ...
  return (
    <div className={styles.card}>
      <div className={styles.cardHeader}>
        <h3>Schedule by Teacher</h3>
        <p className={styles.subtitle}>{teachers.length} teachers · pick one to see their week</p>
      </div>
          ...
          <tbody>
            {slots.map((s) => (
              <tr key={s.idx}>
                ...
                {days.map((d) => {
                  const slot = teacherSlot[d]?.[s.idx];
